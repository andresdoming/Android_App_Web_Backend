...
<?php }
	if (isset($_POST['submit'])){
		...
		if ($biz_name == '' || $category_id == ''){
			...
			renderForm($biz_name, $address, $address2, $city, $zipcode, $phone, $website, $hours, $category_id, $error);
		}
		else{
		
			// inserts the new record into the database
			if ($stmt = $mysqli->prepare("INSERT businesses (biz_name, address, address2, city, zipcode, phone, website, hours, category_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")){
				$stmt->bind_param("ssssssssi", $biz_name, $address, $address2, $city, $zipcode, $phone, $website, $hours, $category_id);
				$stmt->execute();
				$stmt->close();
			}
